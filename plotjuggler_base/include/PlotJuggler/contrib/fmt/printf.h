...
public:
  ...
  void operator()(U value)
  {
    ...
    if (const_check(sizeof(target_type) <= sizeof(int)))
    {
      ...
      {
        arg_ =
            detail::make_arg<Context>(static_cast<int>(static_cast<target_type>(value)));
      }
      else
      {
...
int parse_header(const Char*& it, const Char* end, basic_format_specs<Char>& specs,
                 GetArg get_arg)
{
  int arg_index = -1;
  Char c = *it;
  ...
  if (it != end)
  {
    ...
    else if (*it == '*')
    {
      ...
      specs.width = static_cast<int>(
          visit_format_arg(detail::printf_width_handler<Char>(specs), get_arg(-1)));
    }
  }
  return arg_index;
}
             ...
             basic_format_args<Context> args)
{
  ...
  while (it != end)
  {
    ...
    out = detail::write(
        out, basic_string_view<Char>(start, detail::to_unsigned(it - 1 - start)));
    ...
    if (specs.precision >= 0 && arg.type() == detail::type::cstring_type)
    {
      ...
      arg =
          detail::make_arg<basic_printf_context<OutputIt, Char>>(basic_string_view<Char>(
    ...
    if (arg.is_integral())
    {
      ...
      switch (specs.type)
      {
        ...
        case 'c':
          visit_format_arg(
              detail::char_converter<basic_printf_context<OutputIt, Char>>(arg), arg);
          break;
      }
    }
...
inline auto vsprintf(
    const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args)
...
vfprintf(std::FILE* f, const S& fmt,
         basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> int
{
  basic_memory_buffer<Char> buffer;
  vprintf(buffer, to_string_view(fmt), args);
...
template <typename S, typename Char = char_t<S>>
inline auto vprintf(const S& fmt,
...
template <typename S, typename... T, typename Char = char_t<S>>
FMT_DEPRECATED auto fprintf(std::basic_ostream<Char>& os, const S& fmt, const T&... args)
    -> int
{
