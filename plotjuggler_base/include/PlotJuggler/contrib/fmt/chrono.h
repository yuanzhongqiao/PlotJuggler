...
{
  // See above
  std::size_t (*wcsftime)(wchar_t*, std::size_t, const wchar_t*, const std::tm*) =
      nullptr;
...
template <typename Char, typename Duration>
constexpr Char formatter<std::chrono::time_point<std::chrono::system_clock, Duration>,
                         Char>::default_specs[];
...
FMT_CONSTEXPR const Char* parse_chrono_format(const Char* begin, const Char* end,
                                              Handler&& handler)
{
  auto ptr = begin;
  while (ptr != end)
...
inline int to_nonnegative_int(T value, int upper)
{
  FMT_ASSERT(std::isnan(value) || (value >= 0 && value <= static_cast<T>(upper)), "invali"
                                                                                  "d "
                                                                                  "valu"
                                                                                  "e");
  (void)upper;
...
struct chrono_formatter
{
  ...
  using rep = conditional_t<std::is_integral<Rep>::value && sizeof(Rep) < sizeof(int),
...
public:
  FMT_CONSTEXPR auto parse(basic_format_parse_context<Char>& ctx) -> decltype(ctx.begin())
  {
    ...
    format_str = basic_string_view<Char>(&*range.begin,
                                         detail::to_unsigned(range.end - range.begin));
    return range.end;
  }
  ...
  auto format(const duration& d, FormatContext& ctx) const -> decltype(ctx.out())
  {
    ...
    detail::handle_dynamic_spec<detail::precision_checker>(precision_copy, precision_ref,
                                                           ctx);
    if (begin == end || *begin == '}')
    {
