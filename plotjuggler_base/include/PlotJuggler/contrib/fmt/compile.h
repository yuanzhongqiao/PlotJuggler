...
constexpr auto parse_replacement_field_then_tail(S format_str)
{
  ...
  else if constexpr (c == ':')
  {
    ...
    return parse_tail<Args, result.end, result.next_arg_id>(
        spec_field<char_type, typename field_type<T>::type, ARG_INDEX>{ result.fmt },
        format_str);
  }
}
...
constexpr auto compile_format_string(S format_str)
{
  ...
  if constexpr (str[POS] == '{')
  {
    ...
    {
      static_assert(ID != manual_indexing_id, "cannot switch from manual to automatic "
                                              "argument indexing");
    ...
    else
    {
      ...
      {
        static_assert(ID == manual_indexing_id || ID == 0, "cannot switch from automatic "
                                                           "to manual argument indexing");
        constexpr auto arg_index = arg_id_result.arg_id.val.index;
        return parse_replacement_field_then_tail<get_type<arg_index, Args>, Args,
                                                 arg_id_end_pos, arg_index,
...
constexpr detail::udl_compiled_string<remove_cvref_t<decltype(Str.data[0])>,
                                      sizeof(Str.data) / sizeof(decltype(Str.data[0])),
                                      Str>
operator""_cf()
{
  return {};
